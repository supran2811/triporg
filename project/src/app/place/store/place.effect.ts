...
export class PlacesEffect {
  ...
  getLocation = this.actions$
    .ofType(PlaceActions.GET_CITY_LOCATION)
  ...
  @Effect()
  addPlaceChange = this.actions$
    .ofType(PlaceActions.ADD_PLACE_CHANGE_LISTENER)
    .map((action: PlaceActions.AddPlaceChangeListener) => {
      return action.payload;
    })
    .switchMap(
      (payload: { input: ElementRef; boundary: google.maps.LatLngBounds }) => {
        return this.googlePlaces.addPlaceChangeListener(
          payload.input,
          payload.boundary
        );
      }
  ...
  @Effect()
  saveSelectedPlaceToServer = this.actions$
    .ofType(PlaceActions.SAVE_SELECTED_PLACE_TO_SERVER)
    ...
    .switchMap(([action, placeState, authState]) => {
      ...
      if (savedPlaces.length == 0) {
        return this.http
          .put(url, selectedCity)
          .catch(err => Observable.of(err))
          .switchMap(() => {
            url = url + "/places/" + selectedPlace.placeId;
            return this.http
              .put(url, selectedPlace)
  ...
  @Effect()
  removeSelectedPlaceFromServer = this.actions$
    .ofType(PlaceActions.REMOVE_SELECTED_PLACE_FROM_SERVER)
    .withLatestFrom(this.store.select("place"), this.store.select("auth"))
    .switchMap(([action, placeState, authState]) => {
      const selectedPlace = placeState.selectedPlace;
      const selectedCity = placeState.city;
  ...
  @Effect()
  getSavedPlaceFrmServerByCity = this.actions$
    .ofType(PlaceActions.GET_SAVED_PLACES_FROM_SERVER_BY_CITY)
    .withLatestFrom(this.store.select("place"), this.store.select("auth"))
    .switchMap(([action, placeState, authState]) => {
      const city = placeState.city;
      const uid = authState.uid;
      const url =
        this.USER_SAVE_PLACES_URL + "/" + uid + "/" + city.id + "/places/";
      ...
      return this.http.get<any>(url, null).catch(err => {
  ...
  @Effect()
  getCityDetails = this.actions$
    .ofType(PlaceActions.GET_CITY_DETAILS)
    ...
    .switchMap(([payload, state]) => {
      ...
      if (uid != "") {
        const url =
          this.USER_SAVE_PLACES_URL + "/" + uid + "/" + payload.id + "/";
        return this.http.get<any>(url, null).switchMap(res => {
          if (res === null) {
  ...
  @Effect()
  getPlaceDetails = this.actions$
    .ofType(PlaceActions.GET_PLACE_DETAILS)
    ...
    .mergeMap(([res, state]) => {
      ...
      state.detailsPlace.photos = res.photos
        ? res.photos.map(photo => {
            return {
              small: photo.getUrl({ maxWidth: 200 }),
              large: photo.getUrl({ maxWidth: 800 })
      ...
      state.detailsPlace.opening_text =
        (res.opening_hours && res.opening_hours.weekday_text) || null;
      state.detailsPlace.phoneNumber = res.formatted_phone_number || null;
      state.detailsPlace.reviews = res.reviews
